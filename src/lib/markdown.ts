...
/** Ids are generated by crypto.randomUUID(); imported ones may use letters, digits, "-" and "_". */
const ID_RE = /^[A-Za-z0-9_-]+$/;

function checkId(value: unknown, path: string, errors: Collector): value is string {
  if (typeof value !== 'string' || value.length === 0 || value.length > MAX_ID || !ID_RE.test(value)) {
    errors.add(path, `expected an id of 1 to ${MAX_ID} letters, digits, "-" or "_"`);
    return false;
  }
  return true;
}
